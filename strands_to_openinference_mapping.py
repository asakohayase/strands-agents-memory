...
class StrandsToOpenInferenceProcessor(SpanProcessor):
        ...
        if isinstance(span_id, str):
            try:
                ...
                if self.debug:
                    self._debug_log(
                        "WARN",
                        f"Could not normalize span_id: {span_id}, using hash fallback",
                    )
    ...
    def on_end(self, span: Span):
        ...
        try:
            ...
            if self.debug:
                self._debug_log(
                    "INFO",
                    f"Transformed span successfully",
                    span_name=span.name,
                    original_attrs=len(original_attrs),
                    transformed_attrs=len(transformed_attrs),
                    events=len(events),
                )
            ...
            self._debug_log(
                "ERROR",
                f"Failed to transform span: {str(e)}",
                span_name=span.name,
                span_id=span_id,
            )
    ...
    def _transform_attributes(
        self, attrs: Dict[str, Any], span: Span, events: List = None
    ) -> Dict[str, Any]:
        """
        ...
        if events and len(events) > 0:
            ...
            if prompt or completion:
                input_messages, output_messages = (
                    self._extract_messages_from_attributes(prompt, completion)
                )
            else:
                input_messages, output_messages = [], []
    ...
    def _handle_llm_span(
        ...
        if span_kind == "LLM":
            ...
            if system_prompt:
                ...
                self._debug_log(
                    "INFO",
                    f"Found system prompt in environment variable",
            ...
            if system_prompt:
                ...
                self._debug_log(
                    "INFO",
                    f"Set system prompt for LLM span",
                ...
                if input_messages:
                    ...
                    if not has_system_message:
                        ...
                        self._debug_log(
                            "INFO", f"Added system prompt as first input message"
                        )
                else:
                    # No input messages, create with just system prompt
                    input_messages = [
                        {"message.content": system_prompt, "message.role": "system"}
                    ]
                    self._debug_log(
                        "INFO", f"Created input messages with system prompt"
                    )
            else:
                # Set a placeholder to ensure the attribute exists
                result["llm.prompt_template.template"] = (
                    "[System prompt not available - set STRANDS_AGENT_SYSTEM_PROMPT environment variable]"
                )
                self._debug_log(
                    "WARN", f"No system prompt found for LLM span, using placeholder"
                )
        ...
        if span_kind == "LLM" and system_prompt:
            ...
            if "llm.input_messages" in result:
                try:
                    ...
                    self._add_arize_compatible_event(
                        span,
                        updated_input_messages,
                        output_messages,
                        system_prompt,
                        attrs,
                    )
                    ...
                    self._add_arize_compatible_event(
                        span, input_messages, output_messages, system_prompt, attrs
                    )
            else:
                self._add_arize_compatible_event(
                    span, input_messages, output_messages, system_prompt, attrs
        ...
        if span_kind == "LLM" and system_prompt:
            ...
            if "llm.input_messages" in result:
                try:
                    ...
                    final_input_messages = input_messages
            self._add_nested_llm_json_attribute(
                result, final_input_messages, output_messages, system_prompt, attrs
            )
                ...
                if message_content:
                    ...
                    if message:
                        ...
                        if "finish_reason" in event_attrs:
                            message["message.finish_reason"] = event_attrs[
                                "finish_reason"
                            ]
                        output_messages.append(message)
                    ...
                    if isinstance(item, dict):
                        if "text" in item:
                            ...
                            if "content" in tool_result:
                                if isinstance(tool_result["content"], list):
                                    for tr_content in tool_result["content"]:
                                        if (
                                            isinstance(tr_content, dict)
                                            and "text" in tr_content
                                        ):
                            ...
                            if "toolUseId" in tool_result:
                                message["message.tool_call_id"] = tool_result[
                                    "toolUseId"
                                ]
    ...
    def _flatten_messages(
        ...
        for idx, msg in enumerate(messages):
            for key, value in msg.items():
                clean_key = (
                    key.replace("message.", "") if key.startswith("message.") else key
                )
                    ...
                    for tool_idx, tool_call in enumerate(value):
                        if isinstance(tool_call, dict):
                            for tool_key, tool_val in tool_call.items():
                                ...
                                result[tool_dotted_key] = self._serialize_value(
                                    tool_val
                                )
                else:
            ...
            if output_messages:
                ...
                if span_kind == "LLM":
                    ...
                    output_structure = {
                        "choices": [
                            {
                                "finish_reason": last_message.get(
                                    "message.finish_reason", "stop"
                                ),
                                "index": 0,
                                "message": {
                                    "content": content,
                                    "role": last_message.get(
                                        "message.role", "assistant"
                                    ),
                                },
                            }
    ...
    def _handle_tool_span(
        self, attrs: Dict[str, Any], result: Dict[str, Any], events: List = None
    ):
    ...
    def _convert_to_nested_llm_format(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "input_messages" in llm_attrs:
            try:
                input_messages = (
                    json.loads(llm_attrs["input_messages"])
                    if isinstance(llm_attrs["input_messages"], str)
                    else llm_attrs["input_messages"]
                )
                ...
                has_system_message = (
                    ...
                    and input_messages[0].get("message.role") == "system"
                )
                ...
                if (
                    ...
                    and system_prompt
                    != "[System prompt not available - check AGENT span configuration]"
                ):
                    system_message = {
                        "message.content": system_prompt,
                        "message.role": "system",
                    ...
                    input_messages = [system_message] + (
                        input_messages if input_messages else []
                    )
        ...
        if "output_messages" in llm_attrs:
            try:
                output_messages = (
                    json.loads(llm_attrs["output_messages"])
                    if isinstance(llm_attrs["output_messages"], str)
                    else llm_attrs["output_messages"]
                )
        ...
        if "prompt_template.template" in llm_attrs:
            ...
            if "prompt_template.variables" in llm_attrs:
                llm_obj["prompt_template"]["variables"] = llm_attrs[
        ...
        for key, value in llm_attrs.items():
            ...
            if (
                ...
                and not key.startswith("token_count.")
                and key not in simple_attrs
            ):
                # For nested keys like "input_messages.0.message.content", keep them flat
                if (
                    "." in key
                    and not key.startswith("prompt_template")
                    and not key.startswith("token_count")
                ):
                    continue
                llm_obj[key] = value
        ...
        if llm_obj:
            ...
            self._debug_log(
                "INFO",
                f"Converted to nested LLM format",
                nested_keys=list(llm_obj.keys()),
            )

        return result

    def _add_arize_compatible_event(
        self,
        span: Span,
        input_messages: List[Dict],
        output_messages: List[Dict],
        ...
        try:
            ...
            llm_data["prompt_template"] = {"template": f"system: {system_prompt}"}
            ...
            event_data = {"llm": llm_data, "openinference": {"span": {"kind": "LLM"}}}
            ...
            self._debug_log(
                "INFO",
                "Added Arize-compatible nested LLM event",
    ...
    def _add_nested_llm_json_attribute(
        self,
        result: Dict[str, Any],
        input_messages: List[Dict],
        output_messages: List[Dict],
        ...
        try:
            ...
            llm_nested = {
                ...
                "prompt_template": {"template": f"system: {system_prompt}"},
            ...
            self._debug_log(
                "INFO",
                "Added nested LLM data as JSON attributes",
                strategies=4,
                llm_json_length=len(llm_json),
            )

        except Exception as e:
            self._debug_log(
                "ERROR", f"Failed to add nested LLM JSON attribute: {str(e)}"
            )
    ...
    def _filter_arize_attributes(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in result.items():
            ...
            if key.startswith("arize."):
                self._debug_log(
                    "INFO",
                    f"Filtering out arize attribute from LLM span",
                    attribute=key,
                )
                continue
            # Skip openinference.llm attribute
                ...
                self._debug_log(
                    "INFO", f"Filtering out openinference.llm attribute from LLM span"
                )
                continue
